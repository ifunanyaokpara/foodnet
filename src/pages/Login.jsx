...
export default function Login() {
  ...
  const handleSubmit = e => {
    ...
    e.preventDefault()
    try {
      toast.loading(`Please wait...`, {id:"86061" })
      const findUser = allUsers.find(user => user.email === email.toLowerCase() && user.password === password)
      if(findUser){
        toast.success(`Welcome ${findUser.firstname} ${findUser.lastName} `, {id:"86061", duration: "5000" })
      }
      else{
        toast.error(`Invalid username and password. please try again`, {id:"86061", duration: "5000" })
      }
    } catch (error) {
      toast.error(`Something went wrong. Please try again`, {id:"86061", duration: "5000" })
    }
  }
  ...
  return (
    <section className="grid sm:grid-cols-2">
      ...
      <form onSubmit={handleSubmit }  className="bg-white py-10 px-4 mx-auto flex flex-col justify-center gap-2 w-full max-w-sm">
        <label htmlFor="email" className="text-small text-slate-500">Email Address</label>
        ...
        <label htmlFor="password" className="text-small text-slate-500">Password</label>
